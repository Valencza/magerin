    ...
    <div id="notificationButton" name="notification" class="modal" style="display: none;">
      <div class="modal-dialog">
        ...
        <div class="maincontainer">
            <hr>
            <?php
            // Ambil kategori_id dari parameter URL jika ada
            $kategori_id = isset($_GET['kategori_id']) ? $_GET['kategori_id'] : null;

            // Inisialisasi variabel nama_kategori
            $nama_kategori = "Populer"; // Nilai default jika kategori tidak terdefinisi

            // Query database untuk mengambil nama kategori berdasarkan kategori_id
            if ($kategori_id) {
                $query_kategori = mysqli_query($conn, "SELECT nama_kategori FROM tbl_kategori WHERE id_kategori = $kategori_id");
                $kategori = mysqli_fetch_assoc($query_kategori);

                // Periksa apakah nama_kategori telah ditemukan
                if ($kategori) {
            ...
            <div class="container" id="Movies">
                ...
                // Query database untuk mengambil video berdasarkan kategori jika kategori_id ada
                if ($kategori_id) {
                    $query_new = mysqli_query($conn, "SELECT * FROM tbl_video WHERE id_kategori = $kategori_id ORDER BY id_video DESC");
                } else {
                    $query_new = mysqli_query($conn, "SELECT * FROM tbl_video ORDER BY id_video DESC");
                }
        ...
        <script>
            $(document).ready(function() {
                ...
                // Terapkan pencarian saat form di-submit
                $('#searchForm').submit(function(e) {
                    e.preventDefault();
                    var searchText = $('#searchText').val();
                    $('#myTable').DataTable().search(searchText).draw();
                });
            });
        </script>
